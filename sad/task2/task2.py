...
result = minimize_scalar(
    log_likelihood, args=(data,), bounds=(0.01, 10), method="bounded"
)
theta_hat = result.x
...
plt.subplot(1, 2, 2)
plt.step(
    x_values, ecdf_values, where="post", label="Эмпирическая функция распределения"
)
plt.plot(
    x_values,
    stats.norm.cdf(x_values, scale=1 / np.sqrt(2 * theta_true)),
    "b--",
